...
static void genl_error(struct nlerr* msg)
{
	if(msg->err == -ENETDOWN)
		...
		handle_scan_error(msg->err);
	else if(authstate != AS_IDLE)
		handle_auth_error(msg->err);
}
...
void setup_netlink(void)
{
	...
	if((nl80211 = query_family_grps(&nl, family, grps)) < 0)
		fail("NL family %s not found\n", family);
	if(grps[0].id < 0)
		fail("NL no group nl80211.%s\n", grps[0].name);

	if((ret = nl_subscribe(&nl, grps[0].id)) < 0)
		fail("NL cannot subscribe nl80211.%s\n", grps[0].name);
